...
namespace PhoneBookUI.Components
{
    public class MenuViewComponent : ViewComponent
    {
        ...
        {
            string? userEmail = HttpContext.User.Identity?.Name; //Emaile Bakılacak
            TempData["LoggedInUserNameServer"] = null;
            if(userEmail != null)
            {
                var user = _memberManager.GetById(userEmail).Data;
                TempData["LoggedInUserNameServer"] = $"{user.Name} {user.Surname}";
            }
            return View();
        }
    }
